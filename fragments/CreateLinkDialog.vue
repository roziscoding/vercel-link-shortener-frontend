  ...
  <v-dialog :value="value" max-width="400" persistent>
    <v-card>
      ...
      <v-card-text>
        ...
        <v-scale-transition>
          <v-alert v-if="error" border="left" type="error" dense>
            {{ error }}
          </v-alert>
        </v-scale-transition>
      </v-card-text>
      <v-card-actions>
        <v-btn text color="error" :disabled="loading" @click="close"> Cancelar </v-btn>
        <v-spacer />
        <v-btn text color="primary" :loading="loading" @click="create"> Enviar </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
